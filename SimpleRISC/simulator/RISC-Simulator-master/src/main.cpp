...
bool takeinterrupt = false;
char* interruptinFileName =NULL;
...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i< token.size(); i++){
		...
		if (token[i] == "-intt"){
			interruptinFileName = new char[token[i+1].size()];
			strcpy(interruptinFileName, token[i+1].c_str());
			takeinterrupt = true;
		}
	...
	if (takeinterrupt){
		ifstream test_interruptin_file;
		test_interruptin_file.open(interruptinFileName, ios::in);

		if (test_interruptin_file.fail()) {
			cout<<"Error opening INTERRUPT_in file"<<endl;
			cout<<"Use -h for help"<<endl;
			takeinterrupt = false;
			return 1;
		}
		else {
			test_interruptin_file.close();
		}
	}
...
void printhelp(){
	...
	cout<<"-intt [INTERRUPT_IN File name] : Specify INTERRUPT_IN File Name"<<endl;
