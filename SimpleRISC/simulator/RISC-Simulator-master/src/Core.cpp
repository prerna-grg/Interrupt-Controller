...
void Core::write_context() {
	if (specifycontextout){
		...
		for(int i=0; i<16; i++){
			context_file<<"R"<<dec<<i<<" : "<<R[i]<<endl;
		}
		
		context_file<<"SpecialRegisters"<<endl<<endl;
		for(int i=1; i<6; i++){
			context_file<<"P"<<dec<<i<<" : "<<P[i]<<endl;
		}
...
void Core::run_simplesim(){
	...
	int cycle_no=0,int_ID=0;
	ifstream file;
	if( takeinterrupt){
		file.open(interruptinFileName , ios::in);
		if( file >> cycle_no >> int_ID ){}
		else takeinterrupt = false;
	}
	pprint(1)<<"+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+--------+"<<endl;
	...
	while ( checkValidPC(PC.Read()) || ( pipeline && ((if_of.bubble.Read() == false) || (of_ex.bubble.Read() == false) || (ex_ma.bubble.Read() == false) || (ma_rw.bubble.Read() == false)) )){
		...
		//pprint(2)<<"========================"<<endl;
		unsigned int resume = MEM.Read(PC.Read());
		if (inst_bitset(resume,28, 32) == 22 ){
			int g = counter+1+3; //resume cycle
			cout << "Program resumed in Cycle No.: " << g << endl <<  endl;
		}
		...
		if(isException){
			cout << "Exception caused in Cycle No.: " << counter+1 << endl;
			cout << "Servicing begins in Cycle No.: " << counter+2 << endl;
			cout << "Exception caused in Instruction No. " << of_ex.PC.Read()/4 + 1 ;
			cout << " due to " ;
			if( P[5]== 2) cout << "division by zero" << endl;
			else if( P[5]== 4) cout << "data read error" << endl;
			else if( P[5] == 5) cout << "data write error" << endl;
			PC.Write(0x4);
			PC.clock();
			...
			cout << "Interrupt caused in Cycle No.: " << counter+1 << endl;
			cout << "Servicing begins in Cycle No.: " << counter+2 << endl;
			PC.Write(0x8);
			PC.clock();
			unsigned int temp = PC.Read();
...
void Core::decode() {
	...
	of_ex.mark.Write(if_of.mark.Read());
...
void Core::execute() {
	...
	bool mark_inst = of_ex.mark.Read();
...
/*	if( isBranchTaken ){
		ex_ma.nPC.Write(branchPC);
	}else if(checkValidPC(temp_PC+4)){
		ex_ma.nPC.Write(temp_PC+4);
	}
	*/
	//////////   ALU  ///////////
	...
	if (temp_isDiv){
		...
		else temp_aluResult = (unsigned int)( (signed int)temp_A / (signed int)temp_B );
	}
	...
	if( temp_isLd || temp_isSt){
		if (temp_aluResult < 0 || temp_aluResult > MEM_CAPACITY - sizeof(unsigned int)){
			if(temp_isLd) P[5] = 4; //data read error
			else if(temp_isSt) P[5] = 5; //data write error
			mark_inst = true;
		}	
	}
	
	if (mark_inst){
		//storing flags in oldFlags
		if(eq && !gt)P[4] = 1;
		else if(!eq && gt)P[4] = 2;
		else if(eq && gt)P[4] = 3;
		else P[4] = 0;
	...
	if (isInterrupt ){
		...
		// store oldPC
		if( temp_PC==0){
			cout << "Your request for an interrupt cannot be executed as EX stage has a bubble" << endl;
		}else{
			P[1]= temp_PC + 4;	
			if_of.WriteBubble(true);
			of_ex.WriteBubble(true);
...
void Core::mem_access() {
	...
	unsigned int temp_nPC = ex_ma.nPC.Read();
...
void Core::write_back() {
	...
	//unsigned int temp_nPC = ma_rw.nPC.Read();
	...
	if (temp_isWb){
		...
		if( temp_isMovz && temp_addr != 0){
			P[temp_addr] = temp_result;
			fprint(1)<<"; "<< "P" << temp_addr << " = 0x"<<hex<<temp_result;
		}
		else R[temp_addr] = temp_result;
...
bool Core::checkValidPC(unsigned int testPC){
	...
	if (testPC < 0 || testPC > MEM_CAPACITY - sizeof(unsigned int)){
		P[5] = 3; //ID of instruction fetch error
		return false;
	}
